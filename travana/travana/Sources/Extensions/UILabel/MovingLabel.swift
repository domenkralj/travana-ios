...
//
//  Resource from https://github.com/cbpowell/MarqueeLabel/blob/master/Sources/MarqueeLabel.swift
//  and https://github.com/cbpowell/MarqueeLabel
...
open class MovingLabel: UILabel, CAAnimationDelegate {
    ...
    /**
     Defines the direction and method in which the `MovingLabel` instance scrolls.
     `MovingLabel` supports six default types of scrolling: `Left`, `LeftRight`, `Right`, `RightLeft`, `Continuous`, and `ContinuousReverse`.
     ...
     */
    open var type: MovingType = .continuous {
        didSet {
            if type == oldValue {
                return
            }
    ...
    /**
     ...
     - SeeAlso: FadeStep
     */
    open var scrollSequence: [MovingStep]?
    ...
    /**
     A boolean property that sets whether the `MovingLabel` should hold (prevent) automatic label scrolling.
     
     When set to `true`, `MovingLabel` will not automatically scroll even its text is larger than the specified frame,
    ...
    /**
     A boolean property that sets whether the `MovingLabel` should only begin a scroll when tapped.
     
     If this property is set to `true`, the `MovingLabel` will only begin a scroll animation cycle when tapped. The label will
    ...
    @IBInspectable open var tapToScroll: Bool = false {
        didSet {
            if tapToScroll != oldValue {
                if tapToScroll {
                    let tapRecognizer = UITapGestureRecognizer(target: self, action: #selector(MovingLabel.labelWasTapped(_:)))
    ...
    /**
     The `MovingLabel` scrolling speed may be defined by one of two ways:
    ...
    /**
     Defines the speed of the `MovingLabel` scrolling animation.
    ...
    /**
     ...
     This propery sets the size (in points) of the view edge transparency fades on the left and right edges of a `MovingLabel`. The
    ...
    /**
     Convenience method to restart all `MovingLabel` instances that have the specified view controller in their next responder chain.
    
     - Parameter controller: The view controller for which to restart all `MarqueeLabel` instances.
    
     - Warning: View controllers that appear with animation (such as from underneath a modal-style controller) can cause some `MovingLabel` text
    ...
    open class func restartLabelsOfController(_ controller: UIViewController) {
        MovingLabel.notifyController(controller, message: .Restart)
    }
    ...
    open class func controllerViewWillAppear(_ controller: UIViewController) {
        MovingLabel.restartLabelsOfController(controller)
    }
    ...
    /**
     Labelizes all `MovingLabel` instances that have the specified view controller in their next responder chain.
    
     The `labelize` property of all recognized `MovingLabel` instances will be set to `true`.
    ...
    open class func controllerLabelsLabelize(_ controller: UIViewController) {
        MovingLabel.notifyController(controller, message: .Labelize)
    }
    
    /**
     De-labelizes all `MovingLabel` instances that have the specified view controller in their next responder chain.
     
     The `labelize` property of all recognized `MovingLabel` instances will be set to `false`.
     
     - Parameter controller: The view controller for which all `MovingLabel` instances should be de-labelized.
     - SeeAlso: labelize
     */
    open class func controllerLabelsAnimate(_ controller: UIViewController) {
        MovingLabel.notifyController(controller, message: .Animate)
    }
    ...
    /**
     Returns a newly initialized `MovingLabel` instance with the specified scroll rate and edge transparency fade length.
    ...
    /**
     Returns a newly initialized `MovingLabel` instance.
     
     The default scroll duration of 7.0 seconds and fade length of 0.0 are used.
     
     - Parameter frame: A rectangle specifying the initial location and size of the view in its superview's coordinates. Text (for the given font, font size, etc.) that does not fit in this frame will automatically scroll.
     - Returns: An initialized `MovingLabel` object or nil if the object couldn't be created.
    */
    ...
    private func setup() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(MovingLabel.restartForViewController(_:)), name: NSNotification.Name(rawValue: MarqueeKeys.Restart.rawValue), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(MovingLabel.labelizeForController(_:)), name: NSNotification.Name(rawValue: MarqueeKeys.Labelize.rawValue), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(MovingLabel.animateForController(_:)), name: NSNotification.Name(rawValue: MarqueeKeys.Animate.rawValue), object: nil)
        // UIApplication state notifications
        NotificationCenter.default.addObserver(self, selector: #selector(MovingLabel.restartLabel), name: UIApplication.didBecomeActiveNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(MovingLabel.shutdownLabel), name: UIApplication.didEnterBackgroundNotification, object: nil)
    }
    ...
    //
    // MARK: - MovingLabel Heavy Lifting
    //
    ...
    private func beginScroll(_ sequence: [MovingStep]) {
    ...
    private func generateScrollAnimation(_ sequence: [MovingStep]) -> MLAnimation {
    ...
    private func generateGradientAnimation(_ sequence: [MovingStep], totalDuration: CGFloat) -> MLAnimation {
        // Setup
        var totalTime: CGFloat = 0.0
        ...
        let fadeSteps = sequence.enumerated().filter { (arg: (offset: Int, element: MovingStep)) -> Bool in
    ...
    private func applyGradientMask(_ fadeLength: CGFloat, animated: Bool, firstStep: MovingStep? = nil) {
    ...
    private func transactionDurationType(_ labelType: MovingType, interval: CGFloat, delay: CGFloat) -> TimeInterval {
        switch labelType {
 ...
 */
public struct ScrollStep: MovingStep {
    /**
 ...
 */
public struct FadeStep: MovingStep {
    /**
